//
//  convenientFastFoodViewController.m
//  feichacha
//
...
#import "convenientFastFoodHeadCollectionReusableView.h"
#import "convenientFastFoodCollectionViewCell.h"
#import "convenientFastFoodBannerCollectionReusableView.h"

@interface convenientFastFoodViewController ()
{
...
@implementation convenientFastFoodViewController
...
{
    static NSString * CellIdentifier = @"convenientFastFoodCollectionViewCell";
    convenientFastFoodCollectionViewCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:CellIdentifier forIndexPath:indexPath];
    ...
    cell.buyButton.backgroundColor = RGBCOLORA(98, 133, 31, 1);
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (kind == UICollectionElementKindSectionHeader){
        if (indexPath.section == 0) {
            convenientFastFoodBannerCollectionReusableView * headView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"convenientFastFoodBannerCollectionReusableView" forIndexPath:indexPath];
            UIImageView *tempImage = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, SCREENWIDTH, SCREENWIDTH*0.46)];
            [tempImage sd_setImageWithURL:[NSURL URLWithString:@"http://manage.feichacha.com/html/shop/images/fb_gg.jpg"]];
...
-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section{
    if (section == 0) {
        CGSize size = {SCREENWIDTH,SCREENWIDTH*0.46+70};
        return size;
    }else{
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (point.y >= SCREENWIDTH*0.46) {
        frame.origin.y = 64;
    }else{
        frame.origin.y = SCREENWIDTH*0.46+64 - point.y;
    }
...
//    [navButton8 setBackgroundColor:RGBCOLORA(250, 217, 55, 1)];
    [navButton1 setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    ...
    if (point.y >= classHeadViewY1 && point.y < classHeadViewY2) {
        [navButton1 setTitleColor:RGBCOLORA(98, 133, 31, 1) forState:UIControlStateNormal];
    }else if (point.y >= classHeadViewY2 && point.y < classHeadViewY3){
        [navButton2 setTitleColor:RGBCOLORA(98, 133, 31, 1) forState:UIControlStateNormal];
    }else if (point.y >= classHeadViewY3 && point.y < classHeadViewY4){
        [navButton3 setTitleColor:RGBCOLORA(98, 133, 31, 1) forState:UIControlStateNormal];
    }else if (point.y >= classHeadViewY4 && point.y < classHeadViewY5){
        [navButton4 setTitleColor:RGBCOLORA(98, 133, 31, 1) forState:UIControlStateNormal];
    }else if (point.y >= classHeadViewY5 && point.y < classHeadViewY6){
        [navButton5 setTitleColor:RGBCOLORA(98, 133, 31, 1) forState:UIControlStateNormal];
    }else if (point.y >= classHeadViewY6 && point.y < classHeadViewY7){
        [navButton6 setTitleColor:RGBCOLORA(98, 133, 31, 1) forState:UIControlStateNormal];
    }else if (point.y >= classHeadViewY7 && point.y < classHeadViewY8){
        [navButton7 setTitleColor:RGBCOLORA(98, 133, 31, 1) forState:UIControlStateNormal];
    }else if (point.y >= classHeadViewY8 ){
        [navButton8 setTitleColor:RGBCOLORA(98, 133, 31, 1) forState:UIControlStateNormal];
    }
    
}

-(void)initNavigationView{
    navigationView = [[UIView alloc]initWithFrame:CGRectMake(0, SCREENWIDTH*0.46+64, SCREENWIDTH, 78)];
    navigationView.backgroundColor = [UIColor whiteColor];
    navigationView.layer.borderColor = RGBCOLORA(98, 133, 31, 1).CGColor;
...
-(void)buyButton:(UIButton *)sender{
    ...
    convenientFastFoodCollectionViewCell *cell = (convenientFastFoodCollectionViewCell *)[_collectionView cellForItemAtIndexPath:indexpath];
